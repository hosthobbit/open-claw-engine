...
Go to https://github.com/hosthobbit/open-claw-engine.
...
Copy the jarvis-content-engine folder into your WordPress wp-content/plugins directory.
In wp-admin → Plugins, activate Open Claw Engine.
Go to Open Claw Engine in the admin menu to configure:
...
How to use
First-time setup
Open the settings page In the WordPress admin sidebar, click Open Claw Engine. You'll see two tabs: Settings and Job Logs.
...
Set connection and auth (Mode A)
...
Via REST API (Mode A) Your external agent calls:
...
Via WordPress (scheduled) If you use Publish cadence (e.g. daily at 03:00), WP-Cron will run the configured job automatically. Check Job Logs after the run.

Editorial workflow
Draft-only (recommended) With Draft only enabled, every generated post is saved as a draft. You review it in Posts, edit if needed, then click Publish when ready.

Approve via API For jobs created via the API, you can publish the associated draft by calling POST /wp-json/jarvis/v1/jobs/{id}/approve (or publish the post manually in wp-admin).

Job Logs Use the Job Logs tab to see each run: status, SEO/readability scores, image and link results, and error messages. Fix any misconfiguration (e.g. API key, model, or image URLs) and re-run if needed.
...
Campaign configuration – you configure a daily subject (e.g. “Managed WordPress security tips”) and defaults in the admin UI.
Scheduling – WP‐Cron triggers a daily event at your configured time.
Generation – for each scheduled run or API call:
...
Scoring & Guardrails – Open Claw Engine computes a simple SEO/quality score and enforces thresholds (readability, link distribution, content length).
Images – image URLs from the generator are downloaded into the Media Library, with alt text set from context, and assigned as featured / inline images as configured.
SEO integration – meta title/description and focus keyword are saved in Rank Math / Yoast fields when available, plus plugin‐specific fallbacks.
Logs & Health – job records and scores are stored in a custom table and surfaced in the admin Job Logs view. A /health endpoint is available for external monitoring.
REST API Overview
...
Authenticating with Application Password / JWT / HMAC.
Calling POST /generate or POST /publish with:
subject
primary_keyword and secondary_keywords[]
audience and intent
Optionally polling GET /jobs/{id} for status, or integrating logs into your observability stack.
